...
def process_json_to_dataframe(input_file_path):
    ...
    for record in data:
        ...
        # Go through links to see if packetstorm is referenced
        packetstorm = False

        if references != []:
            for reference in references:

                # First check if packetstorm is in reference
                if 'packetstorm' in reference:
                    
                    # Then check for other key words
                    if ('exploit' in reference or '/files/' in reference) and 'advisories' not in reference:
                        packetstorm = True
